...
class MatrixOperationsTests {
    ...
    fun matrixMulTest() {
        for (n in 0..9) {
            ...
            println("matrixMulTest seed: $seed")
    ...
    fun matrixTransposeTest() {
        for (n in 0..9) {
            ...
            println("matrixTransposeTest seed: $seed")
            val source = RandomSource.ISAAC.create(seed)
    ...
    @Test
    fun reduceMatrixToVectorTest() {
        for (n in 0..9) {
            val bBuffer = ByteBuffer.wrap(securesRandom.generateSeed(8))
            val seed = bBuffer.getLong()

            println("reduceMatrixToVectorTest seed: $seed")
            val source = RandomSource.ISAAC.create(seed)
    ...
    @Test
    fun broadCastVectorToMatrixTest() {
        for (n in 0..9) {
            val bBuffer = ByteBuffer.wrap(securesRandom.generateSeed(8))
            val seed = bBuffer.getLong()

            println("broadCastVectorToMatrixTest seed: $seed")
            val source = RandomSource.ISAAC.create(seed)
